...
- **TYPE_OF** — a Field, Param, or Variant payload references an Item used as its type. Producer asymmetry (cfdb-062-php-declared-shapes §3.1): Rust (`cfdb-extractor`) synthesizes a stub Item for an unresolved target (`synthesize_referenced_items`); PHP (`cfdb-extractor-php`) is closed-world — an arm outside the workspace yields no edge and no node, carried only in the source's `type_normalized`.
...
- **RETURNS** — an fn Item returns a type Item. Same producer asymmetry as `TYPE_OF`: PHP (`cfdb-extractor-php`, cfdb-062-php-declared-shapes §3.1) is closed-world, no edge and no node for an out-of-workspace return-type arm.
...
- **EXTENDS** — the class-like Item on the `extends` side of a `base_clause` points at the `:Item` of each name that resolves in-workspace (cfdb-062-php-declared-shapes#3.2). Closed-world, same resolution pass as `IMPLEMENTS`, and never overlapping it on one clause. Emitted by `cfdb-extractor-php` alone. Attributes: resolver. SchemaVersion V0_8_0+.
...
- **Crate** — a compilation unit of the walked workspace: a Cargo package from `cfdb-extractor`, or the one synthetic crate each of `cfdb-extractor-php` and `cfdb-extractor-ts` emits for the workspace it walks. `is_workspace_member` is written `true` by all three; `published_language` by the Rust and TypeScript producers; `language` by the TypeScript producer alone; `path` by nobody (issue #693). Attributes: crate_tier, is_workspace_member, language, name, path, published_language, version
- **Module** — a namespacing unit of the walked workspace: a Rust `mod` block or file-level module, a PHP `namespace`, a TypeScript module. The attribute sets are disjoint between producers, as on `:File` and `:Item` — `name` from the Rust and PHP producers, `is_test` from the Rust one, `path` (the namespace with `\` rewritten `::`) from the PHP one (issue #693). Attributes: crate, file, is_inline, is_test, name, path, qpath
...
- **Item** — a top-level item (struct, enum, trait, impl, fn, const, static, type alias). `return_type_path` / `return_type_normalized` are emitted by `cfdb-extractor-php` alone, on `fn` items, absent when the declaration carries no return type (cfdb-062-php-declared-shapes §3.1). A `kind:"const"` `:Item` from `cfdb-extractor-php` carries `value_text`, the verbatim bytes of its value expression, and — when the const is typed — `type_normalized` / `type_path` resolved the same way `:Field` and `:Param` resolve theirs; absent on every non-PHP or non-const item (cfdb-062-php-declared-shapes §3.6). Attributes: bounded_context, cfg_gate, crate, cyclomatic, deprecation_since, doc_text, dup_cluster_id, file, git_commit_count, git_last_author, git_last_commit_unix_ts, impl_target, impl_trait, is_deprecated, is_test, kind, language, line, module_qpath, name, php_construct, qname, reachable_entry_count, reachable_from_entry, reachable_from_production_entry, reachable_production_entry_count, return_type_normalized, return_type_path, signature, signature_hash, test_coverage, ts_construct, type_normalized, type_path, unwrap_count, value_text, visibility
- **Field** — a struct field, tuple-struct element, or enum-variant field from `cfdb-extractor`; a PHP class property, or a promoted constructor parameter (which is both a `:Field` and a `:Param`), from `cfdb-extractor-php` (cfdb-062-php-declared-shapes §3.1). `type_normalized` / `type_path` are absent when the declaration carries no type — every Rust field has one, so this only occurs for a PHP producer. A promoted parameter's `index` continues after every property declared in the class body. `default_text`, PHP only, is the verbatim bytes of the property's or promoted parameter's `default_value` field, absent when the declaration carries no default (cfdb-062-php-declared-shapes §3.6). Attributes: default_text, index, name, parent_qname, type_normalized, type_path
- **Variant** — an enum variant. Attributes: index, name, parent_qname, payload_kind
- **Param** — a function or method parameter (Rust); a PHP method or function parameter from `cfdb-extractor-php`, `is_self` always `false` (PHP has no receiver parameter) and `type_normalized` / `type_path` absent when the declaration carries no type (cfdb-062-php-declared-shapes §3.1). `default_text`, PHP only, is the verbatim bytes of the parameter's `default_value` field, absent when the parameter carries no default (cfdb-062-php-declared-shapes §3.6). Attributes: default_text, index, is_self, name, parent_qname, type_normalized, type_path
- **CallSite** — one concrete call expression (caller → callee, file:line). `callee_last_segment` is the cheap name fence three of this repo's own example queries read, emitted by all four producers; `field` names the struct field whose `#[serde(default)]` produced a `serde_default` site, Rust-only (issue #693). `kind` names four shapes — `call`, `fn_ptr`, `serde_default` and `new`, the last a PHP `object_creation_expression` resolving to the class's `__construct` (cfdb-060-php-fact-model#3.2); the set is extended and never narrowed, a producer emitting a subset being the range of that producer and not of this label. Attributes: arg_count, callee_last_segment, callee_path, callee_resolved, caller_qname, field, file, is_test, kind, line, resolver
...
- **Supertype** — one declared `extends` or `implements` name of a class-like Item, as written, emitted by `cfdb-extractor-php` (cfdb-062-php-declared-shapes#3.2). It records a name and never resolves it away: no `:Item` is invented for a target outside the walked tree, so a vendor supertype (`PDO`, `Psr\Log\LoggerInterface`) is recorded as written even though it never gains an `EXTENDS` edge. Id `supertype:{class qname}#{idx}` via `cfdb_core::qname::supertype_node_id`, `idx` the zero-based declaration order with `extends` names first. SchemaVersion V0_8_0+. Attributes: file, fqn, line, relation, written
- **GlobalRead** — a read or write of a PHP superglobal (`GLOBALS`, `_SERVER`, `_GET`, `_POST`, `_FILES`, `_COOKIE`, `_SESSION`, `_REQUEST`, `_ENV`) inside a walked fn or method body (cfdb-062-php-declared-shapes#3.7). Not a `:CallSite` — a superglobal access has no callee. Node id `globalread:{caller_qname}:{name}:{idx}` via `cfdb_core::qname::global_read_node_id`, `idx` the zero-based ordinal among reads of that name in that caller. Emitted by `cfdb-extractor-php` only. SchemaVersion V0_8_0+; keyspaces predating this slice carry zero GlobalRead nodes. Attributes: caller_qname, file, line, name
