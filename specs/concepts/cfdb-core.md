...
- **Item** — a top-level item (struct, enum, trait, impl, fn, const, static, type alias). `return_type_path` / `return_type_normalized` are emitted by `cfdb-extractor-php` alone, on `fn` items, absent when the declaration carries no return type (cfdb-062-php-declared-shapes §3.1). Attributes: bounded_context, cfg_gate, crate, cyclomatic, deprecation_since, doc_text, dup_cluster_id, file, git_commit_count, git_last_author, git_last_commit_unix_ts, impl_target, impl_trait, is_deprecated, is_test, kind, language, line, module_qpath, name, php_construct, qname, reachable_entry_count, reachable_from_entry, reachable_from_production_entry, reachable_production_entry_count, return_type_normalized, return_type_path, signature, signature_hash, test_coverage, ts_construct, unwrap_count, visibility
- **Field** — a struct field, tuple-struct element, or enum-variant field from `cfdb-extractor`; a PHP class property, or a promoted constructor parameter (which is both a `:Field` and a `:Param`), from `cfdb-extractor-php` (cfdb-062-php-declared-shapes §3.1). `type_normalized` / `type_path` are absent when the declaration carries no type — every Rust field has one, so this only occurs for a PHP producer. A promoted parameter's `index` continues after every property declared in the class body. Attributes: index, name, parent_qname, type_normalized, type_path
- **Variant** — an enum variant. Attributes: index, name, parent_qname, payload_kind
- **Param** — a function or method parameter (Rust); a PHP method or function parameter from `cfdb-extractor-php`, `is_self` always `false` (PHP has no receiver parameter) and `type_normalized` / `type_path` absent when the declaration carries no type (cfdb-062-php-declared-shapes §3.1). Attributes: index, is_self, name, parent_qname, type_normalized, type_path
