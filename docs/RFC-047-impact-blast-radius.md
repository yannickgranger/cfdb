...
> **⚠️ SUPERSEDED by [RFC-047a](RFC-047a-impact-query-mechanics.md) (council-ratified 2026-06-25).** The "no list-binding path exists" finding below is **retracted** — that path ships (RFC-047a §1). The actual blockers are open-range parse (`*1..`), a silent depth-5 cap, and the dogfood needing HIR-resolved CALLS. See RFC-047a for the corrected mechanics and the re-cut 47-0/47-A.
