---
title: RFC-043 — `:CallSite` argument facts for receiver-type fences
status: Ratified (4-lens round 2 — clean-arch RATIFY direct; ddd/solid/rust-systems conditional-RATIFY-on-editorial-fixes (5 stale-string sites missed in round-1 amendment sweep) — all 5 sites fixed in round 2 amendment 2026-05-26; ready for operator handoff to upstream PR per §2.4)
date: 2026-05-26
authors: agentry-captain-2026-05-26 (drafted after spec-first + code discovery; clean-arch, ddd-specialist, solid-architect, rust-systems review pending per upstream CLAUDE.md §2.3)
companion: agentry EPIC #793 (consumer-side ratified RFC at agentry:docs/rfc/RFC-verb-coverage-harvest.md, 2026-05-22 4-lens council). Sibling upstream graph-specs RFCs: RFC-005 (verb-coverage report), RFC-006 (verb-anchoring).
...
The agentry consumer RFC §8 names this as **B1 cross-dogfood**: cfdb `:CallSite` receiver-typed edges. The §8 text frames it as "needed only if the `DedicatedConn` newtype is rejected due to orphan-rule + caller-cascade caveat" — i.e., a fallback for the Tier-0 path that fails. But the broader argument-extraction capability is reusable for any future "what does this call receive" fence, so RFC-043 frames it as a general extension, not solely as the B1 fallback.

## §2 — Scope

In scope:

1. New `Label::ARGUMENT` and `EdgeLabel::HAS_ARG` string constants on the existing open-class newtype impls in `cfdb-core/src/schema/labels.rs` (per solid §5.3 BLOCKER 2: cfdb uses `Label(pub String)` + `EdgeLabel(pub String)` newtypes, NOT `#[non_exhaustive]` enums — additive non-breaking by virtue of open-class string vocabulary, not enum-variant additivity). `(:CallSite)-[:HAS_ARG]->(:Argument {position})` connects each call site to its positional arguments (per DDD §5.2 NIT: `position` is a node attribute on `:Argument`, NOT an edge property — preserves vocabulary consistency with `:Param.index` `nodes.rs:237`, `:Field.index` `nodes.rs:169`, `:Variant.index` `nodes.rs:211`).
...
5. Schema bump: `cfdb-core::SchemaVersion` bumps from `V0_4_0` (current per `labels.rs:425`) to `V0_5_0` (new label class warrants minor bump per the precedent: V0_2_0 entry points, V0_3_0 schema-producer alignment, V0_4_0 literals — per solid §5.3 BLOCKER 1 + rust-systems §5.4 finding 3). Consumers (graph-specs cross-dogfood pin, agentry `.cfdb/cfdb.rev`) bump in lockstep PRs per RFC-002 §4.
...
`cfdb-core/src/schema/labels.rs` (verified at the path) gains new constants on the existing open-class newtype impls (NOT enum variants — per solid §5.3 BLOCKER 2):
...
- `pub const RECEIVER_POSITION: u32 = 0;` (per solid §5.3 finding 4) as a sibling constant in cfdb-core's schema module — the stable named reference for the method-call-receiver convention. Cypher rule authors reference this constant conceptually; future cypher tooling MAY codegen against it.
- Both labels gain descriptors in `cfdb-core/src/schema/describe/{nodes,edges}.rs` (per solid §5.3 descriptor-centralization confirmation: existing pattern already routes both extractors through these single descriptor sources).

The `position: u32` attribute lives on `:Argument` itself (per DDD §5.2 NIT), NOT on the `HAS_ARG` edge. This mirrors `:Param.index`, `:Field.index`, `:Variant.index` placement.

**Shared classifier helper (per clean-arch §5.1 finding 2):** because `syn` types cannot enter `cfdb-core` (RFC-032 §3 boundary precedent for `ra_ap_*` applies equally to `syn`), the `kind` classifier helper lives in a NEW internal crate `cfdb-extractor-shared` that is depended on by both `cfdb-extractor` and `cfdb-hir-extractor`. Function signature: `pub fn classify_arg_kind(expr: &syn::Expr) -> &'static str` returns one of the closed-set strings `"path" | "method_call" | "call" | "ref" | "literal" | "other"`. Both extractors call this single function — no per-extractor classification logic.

**Node-ID formula (per clean-arch §5.1 finding 3 + rust-systems §5.4 finding 4):** `cfdb-core/src/qname/node_id.rs` gains `pub fn argument_node_id(callsite_id: &str, position: u32) -> String` returning `format!("arg:{callsite_id}#{position}")`. The callsite_id is already resolver-scoped (per the existing `cs_id` formula in `call_visitor.rs:175` and `call_site_emitter.rs:328`), so `:Argument` node IDs INHERIT the resolver scope. **Cross-extractor `:Argument` identity is NOT a Slice A design goal** — syn-emitted `:CallSite` and HIR-emitted `:CallSite` already have divergent IDs by design (per RFC-032 §3 resolver-discriminator contract), and their `:Argument` children inherit that divergence. See Invariant 9 below.
...
3. **No HIR `TYPE_OF` from `:Argument`** in Slice A. Adding `TYPE_OF` would force the syn extractor to emit incomplete facts (no types) which breaks the resolver-discriminator contract from RFC-032 §3 (`resolver: "syn" | "hir"` on `:CallSite`). The HIR-only type resolution is a separate RFC.
...
- (CONFIRMED) `:Argument` lives in `cfdb-core` (vocabulary layer).
- (BLOCKING) Shared `kind` classifier — **RESOLVED (§3.1):** new `cfdb-extractor-shared` crate holds `classify_arg_kind`; both extractors call the same pure helper. `syn` types stay quarantined out of cfdb-core per RFC-032 §3 precedent.
- (BLOCKING) `argument_node_id` formula — **RESOLVED (§3.1):** `format!("arg:{callsite_id}#{position}")` added to `cfdb-core/src/qname/node_id.rs` in Slice A scope.

**ROUND 2 VERDICT (clean-arch): RATIFY.** Both round-1 BLOCKERs cleanly resolved. `cfdb-extractor-shared` placement honors RFC-032 §3 `syn`-quarantine boundary. `argument_node_id` formula matches existing `param_node_id` / `field_node_id` shape. Implementation-phase observation noted: HIR-native classifier escape hatch requires shared-fixture-set equivalence test before use.

Open questions for the lens:
- `:Argument` is a new node label — does it belong in `cfdb-core` (the vocabulary layer) or warrant a sub-layer? cfdb-core is already the home for `:CallSite`, so consistency favors core. Confirm.
- Both syn and HIR extractors emit the new facts. Is the parallel emission code structured as a shared module in `cfdb-core` (or a shared crate) to avoid divergence, or duplicated per extractor?
- Cypher consumers query `(:CallSite)-[:HAS_ARG]->(:Argument)`. Should `:Argument` nodes be globally unique or scoped to their `:CallSite`? (Affects node-ID derivation.)

### §5.2 — Domain-driven design

**REQUEST CHANGES** (round 1) — folded.
- (BLOCKING) `:Argument` lifecycle invariant absent — **RESOLVED (§4 Invariant 8):** parallel to RFC-036 CP4 for `:EntryPoint`; convention-enforced via `argument_node_id` prefix.
- (REQUEST CHANGES) `kind="other"` cypher prohibition — **RESOLVED (§4 Invariant 10 + §3.1 descriptor description):** explicit.
- (BLOCKING) Resolver inheritance contract — **RESOLVED (§3.3 + §4 Invariant 9):** discriminated coexistence — syn-emitted `:Argument` attached to syn `:CallSite`; HIR-emitted attached to HIR `:CallSite`; no merge.
- (NIT) `position` ordinality placement — **RESOLVED (§2 + §3.1):** moved to `:Argument` node attribute (parallel to `:Param.index`).

**ROUND 2 VERDICT (ddd): RATIFY** (conditional-on-editorial-fixes; all 5 sites fixed in round-2 amendment). All round-1 findings cleanly resolved. The §3.4 wire-format sentence and §3.5 cypher example were amended to match the node-attribute `position` decision. No new DDD findings.

Open questions for the lens:
- `:Argument` is a value object owned by `:CallSite`. The aggregate root is `:CallSite`; deleting a `:CallSite` should delete its `:Argument`s. Does cfdb-core's graph model already enforce this lifecycle coupling, or is it implicit?
- The `kind` discriminator (`"path"` / `"method_call"` / `"call"` / `"ref"` / `"literal"` / `"other"`) is a Ubiquitous Language artifact this RFC introduces. Is "other" a code smell (incomplete enumeration) or honest open-class signaling?
- Cross-resolver consistency: syn and HIR may classify the same argument expression differently (HIR has more type info). The RFC mandates "same schema" but does it mandate "same classification"? If syn says `"path"` and HIR says `"method_call"` for the same expression, which wins, and what's the contract?

### §5.3 — SOLID + component principles

**REQUEST CHANGES** (round 1) — folded.
- (BLOCKING) SchemaVersion bump `V0_2_X → V0_2_(X+1)` was stale — **RESOLVED:** corrected to `V0_4_0 → V0_5_0` throughout (verified `CURRENT = V0_4_0` at `labels.rs:425`).
- (BLOCKING) `#[non_exhaustive]` enum-variant language was factually wrong — **RESOLVED:** replaced with open-class newtype constant language (`Label(pub String)` + `EdgeLabel(pub String)` precedent).
- (CONDITION) Descriptor centralization — **CONFIRMED:** both extractors route through `cfdb-core/src/schema/describe/{nodes,edges}.rs` single sources.
- (RECOMMEND) `RECEIVER_POSITION` convention constant — **RESOLVED (§3.1):** `pub const RECEIVER_POSITION: u32 = 0;` added to `cfdb-core`.
- (NO CHANGE) §4 Invariant 7 atomicity — sufficient.

**ROUND 2 VERDICT (solid): RATIFY** (conditional-on-editorial-fixes; both stale-string blockers fixed in round-2 amendment — §3.4 SchemaVersion + §4 Invariant 1 `#[non_exhaustive]` language). `RECEIVER_POSITION` convention constant + descriptor centralization + open-class newtype framing all correctly amended. Stale open-question in §5.3 panel converted to resolution annotation.

Open questions for the lens:
- ~~The schema bump is `V0_2_X → V0_2_(X+1)` — confirm against the actual current cfdb-core `SchemaVersion` constant.~~ **RESOLVED:** corrected to `V0_4_0 → V0_5_0` (current `CURRENT = V0_4_0` at `labels.rs:425`).
- The new `:Argument` label crosses syn and HIR extractor boundaries. SDP/SAP: is the descriptor centralized (one source of truth in cfdb-core) or replicated per extractor (drift risk)?
- The cypher example uses `position: 0` for method-call receivers — is this convention spec'd anywhere outside this RFC? Future cypher rule authors need a stable convention. Should it land as a constant in cfdb-core?

### §5.4 — Rust systems

**REQUEST CHANGES** (round 1) — folded.
- (Q1) `source_text` cost acceptable for Slice A — **CONFIRMED:** linear, not quadratic; future RFC may add `--no-arg-source-text` if profiling warrants.
- (Q2) `proc-macro2 span-locations` feature pinned — **CONFIRMED** at workspace `Cargo.toml:47`.
- (BLOCKING) HIR-side `:Argument` emission algorithm unspecified — **RESOLVED (§3.3):** `ast::ArgList` traversal + `LineIndex` line extraction + shared `classify_arg_kind` helper. Algorithm spelled out concretely.
- (BLOCKING) Cross-resolver `kind` precedence rule undefined — **RESOLVED (§3.3 + §4 Invariant 9):** discriminated coexistence — both extractors emit separately under their resolver-discriminated `:CallSite` parents; cypher author filters on parent's `resolver`.
- (BLOCKING) SchemaVersion stale — **RESOLVED:** `V0_4_0 → V0_5_0`.
- (FINDING) `:Argument` node-ID resolver-scoping — **RESOLVED (§4 Invariant 9):** explicit non-cross-extractor-stability.
- (FINDING) `source_text` optional-flag — **RESOLVED (§6 Non-goals):** explicit future-RFC note.

**ROUND 2 VERDICT (rust-systems): RATIFY** (conditional-on-editorial-fix; §3.4 line 84 SchemaVersion stale-string fixed in round-2 amendment). All other amendments correctly folded — HIR algorithm concrete, discriminated coexistence resolves precedence cleanly, resolver-scoped node IDs explicitly out-of-scope for cross-extractor identity, source_text optional-flag deferred to future RFC with concrete escape hatch.

Open questions for the lens:
- `arg.to_token_stream().to_string()` cost: each `:Argument` materializes a String per argument. Large workspaces have millions of call sites × N arguments each. Is the schema bump worth the extraction cost? Should `source_text` be optional (gated by an extractor flag)?
- `proc-macro2` `span-locations` feature: confirm it's already pinned in the workspace (cfdb-extractor's Cargo.toml).
- HIR-extractor parity: HIR represents call expressions via `ra_ap_hir::Expr` (or similar) — does the same `:Argument` shape map cleanly, or does HIR force a different node-ID derivation algorithm?
- Node-ID stability for `:Argument`: cfdb's existing node-ID convention (e.g., `cfdb-core::qname::callsite_node_id`) needs an analog for `:Argument`. Slice A scope.
...
- A separate `cfdb-arguments` crate. The new vocabulary lives in cfdb-core; emission lives in the existing extractor crates. **One new internal crate `cfdb-extractor-shared` IS added** (per clean-arch §5.1 finding 2) to hold the `classify_arg_kind` pure helper — `syn` types cannot cross into `cfdb-core` per RFC-032 §3 boundary; the shared crate is the composition root for syn-typed shared code between the two extractor crates.
...
- Updating any downstream consumer (graph-specs, agentry) in this PR. Per RFC-002 §4, lockstep PRs land within 24h; this RFC's PR is the upstream half.

## §7 — Issue decomposition

Two vertical slices.

### Slice A — schema vocabulary + syn-extractor emission + HIR-extractor parity + SchemaVersion bump
...
### Slice B — downstream lockstep coordination
...
For the Tier-0 alternative (the `DedicatedConn` newtype path agentry RFC §8 also names): that path retires the text-grep fences differently (via Rust type system). Both paths are valid; choosing between them is operator/architect decision per the agentry-side conditional. RFC-043 ships the cypher path; the newtype path proceeds independently.
...
- cfdb RFC-032 §3 (HIR-extractor boundary): `docs/RFC-032-v02-extractor.md` — the resolver discriminator contract this RFC preserves.
- cfdb RFC-002 §4 + RFC-033 (cross-dogfood with graph-specs): cross-fact locking + lockstep schema bump protocol.
