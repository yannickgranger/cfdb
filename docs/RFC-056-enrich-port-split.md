...
- Refs: cfdb self-audit 2026-08-17 (session `220c95c4`, artifact `cfdb Self-Audit`) — major finding: `cfdb-petgraph` is the repo's big-ball-of-mud. Characterization safety net: PR #575 (commit `c21f89b` on develop) pins the current `EnrichBackend` dispatch surface byte-for-byte. Precedent: cfdb-031-audit-cleanup (original `EnrichBackend` / `StoreBackend` port split).
...
`KeyspaceState implements GraphView` is a clean Anti-Corruption Layer, no Shared-Kernel risk between `cfdb-enrich` and `cfdb-petgraph`. Noted, not changed: "View" conventionally connotes read-only (CQRS) in DB/DDD vocabulary while `GraphView` is read+write; the RFC's own doc comment already self-corrects this ("read/write surface"), sufficient to prevent misreading.

### 5.3 SOLID / component principles (`solid-architect`) — R1: REQUEST CHANGES → folds applied → R2: RATIFY

SDP direction preserved on `cfdb-core` (Ce=0 before/after); 056-F's pass grouping (`reachability` + `attr_call_resolution`) is real CCP-correct coupling, not arbitrary (`reachability.rs:154` calls into `attr_call_resolution` directly); ISP improvement is the RFC's strongest, quantified result (9 `pub(crate)` methods + 3 raw petgraph-typed fields today → 6 dyn-safe id-based methods, zero petgraph types in any pass signature).

### 5.4 Rust systems (`rust-systems`) — R1: REQUEST CHANGES → folds applied → R2: RATIFY

the composition root is a plain `match` over a concrete `PetgraphStore` (never boxed/dyn-dispatched), so flipping arms per-slice is a monomorphized call with the `Send + Sync` bound checked once, statically, at the `impl EnrichBackend for EnrichEngine<'s, S>` site — independent of when call sites route through it.
...
- **Further CRP splitting of `cfdb-core`'s port-trait family from its data-model types.** Named, not fixed, per solid R1: only 2 of 12 intra-workspace consumers of `cfdb-core` use any port trait (`StoreBackend`/`EnrichBackend`) today; this RFC extends that existing pattern (2 traits → 4) rather than introducing it (cfdb-031-audit-cleanup precedent), and does not attempt to further separate `cfdb-core`'s port-trait surface from its DTO surface. A future RFC's problem, if the ratio ever becomes load-bearing.
