...
- Status: **RATIFIED 4/4** (2026-08-17, R2) — clean-arch · ddd-specialist · solid-architect · rust-systems. R1: 4× REQUEST CHANGES (independently convergent on the composition-root sequencing bug) → author fold → R2: 4/4 RATIFY. Verdict record: `council/RFC-056/RATIFIED.md`.
...
// for the trait suffix that was itself renamed away from "Port")
...
**Implementation note (ddd R2, non-blocking):** `schema.rs`'s module doc currently names its consumers as "the extractor, parser, and evaluator"; the 056-0 PR should add enrichment as a fourth named consumer and state which submodule `Direction` lives in, so the doc stays accurate once `cfdb-enrich` depends on it.
...
|---|---|---|---|
| 056-0 | Port + `GraphBackend`/`GraphView` in `cfdb_core::graph`, `Direction` relocation (§3.3), `cfdb-enrich` scaffold + `EnrichEngine` shell + `[dev-dependencies] cfdb-petgraph` wiring, `enrich_deprecation` (trivial, no port use) rides along | **`EnrichVerb::Deprecation` → `EnrichEngine`** (amended post-ratification, implementation finding — see note below); `cfdb-cli` still calls `PetgraphStore` for the other 6 verbs | Additive only — zero behavior change, zero *real* pass moved yet. Proves the port compiles and delegates correctly before any real pass depends on it. |
...
Each slice A–F: move the pass module + its tests into `cfdb-enrich`, rewrite against `GraphView`, add the `EnrichEngine` dispatch arm, delete the old arm from `cfdb-petgraph::enrich_backend.rs`, **and flip that verb's `cfdb-cli/src/enrich.rs` dispatch arm in the same PR** (§2). The PR #575 characterization test for that verb (plus 056-0's port-delegation tests) is the acceptance gate — must pass with **zero assertion changes**. Self-dogfood: `cfdb extract --workspace .` + the migrated `enrich-<verb>` **run through the real `cfdb` binary** (`tools/dogfood-enrich` drives exactly this, subprocess-invoking the shipped binary — confirmed at `tools/dogfood-enrich/src/runner.rs`) on cfdb-self, diff report JSON against the pre-slice baseline — must be empty, except 056-F's named perf measurement above.

**Implementation note, added post-ratification (056-0, commits 217e523/7d740df, issue #577):** the "pre-slice baseline (captured once at 056-0)" language above was not implemented as a separate checked-in JSON artifact — 056-0 instead confirmed the pre-existing `crates/cfdb-cli/tests/self_dogfood_enrich_*.rs` suite (in-process, `PetgraphStore` construction) still passes unchanged, and that IS each slice's "before" baseline (git history is the diff target, not a frozen file — avoids a §4 no-ratchet-adjacent baseline-file smell). Two consequences for 056-A onward, found during 056-0's review: (1) those existing tests never exercise `crates/cfdb-cli/src/enrich.rs` — the composition-root dispatcher this RFC requires each slice to flip — so each slice A–F must ALSO add a real-binary CLI test (`assert_cmd::Command::cargo_bin("cfdb")`, pattern: `crates/cfdb-cli/tests/enrich_deprecation_cli.rs`) against **cfdb's own tree**, captured in the same PR *before* the cutover lands, so there is something to diff against; not a synthetic fixture — 056-0's own CLI test uses one, which is safe only because `enrich_deprecation`'s report is content-independent, and is NOT the template for content-dependent passes. (2) 056-F (`reachability`) has no pre-existing self-dogfood test at all yet — that baseline has to be authored from scratch when 056-F's PR captures it, not assumed to already exist.

**Implementation note, added post-ratification (056-A, issue #578):** the two guards in each moved pass have a **load-bearing order**, and it is not the order the port's borrow rules push you toward. Every pre-move `PetgraphStore::enrich_*` arm runs `require_keyspace` FIRST and `require_workspace` second, so a call with an unknown keyspace AND no `workspace_root` attached returns `Err(StoreError::UnknownKeyspace)` — not the degraded `Ok(EnrichReport{ran: false})`. Writing the moved version as "`require_workspace` → `graph_view`" (the shape the borrow checker makes easiest, since `require_workspace` takes `&self` and `graph_view` takes `&mut self`) silently inverts that and returns the degraded report instead — a §4 behavior-identity violation. 056-A shipped the inversion and caught it only in post-commit review, because no pre-existing fixture failed both guards at once: `unknown_keyspace_returns_err` attaches a workspace root, `no_workspace_root_returns_degraded_report` registers the keyspace, so each guard was pinned individually and their order was pinned by nothing. The fix is to resolve and discard the view first (`let _ = self.store.graph_view(keyspace)?;` — the borrow ends with the statement, freeing `require_workspace` to run), and to pin it with a both-guards-fail test; see `EnrichEngine::enrich_rfc_docs` and `unknown_keyspace_errs_even_when_workspace_root_is_also_missing` for the worked pattern. **This applies to every remaining slice that reads `workspace_root` — 056-B (`bounded_context`), 056-C (`concepts`), 056-D (`git_history`), 056-E (`metrics`), 056-F (`reachability`) — each must preserve the ordering and carry its own both-guards-fail test.** (`enrich_deprecation`, moved in 056-0, is exempt: it takes no workspace root, so it has a single guard and no order to invert.)
