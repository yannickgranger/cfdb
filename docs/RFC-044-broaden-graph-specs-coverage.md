...
**Amendment (author-documented, 2026-09-07, cfdb #711):** §7.8 Tests · Unit — the sentence "Each Cypher file's header comment documents which RFC §N invariant it encodes and (for positive invariants expressed as their negation) names the inversion" is RETIRED by the operator's comment ruling of 2026-08-19, which is later than this RFC's ratification and admits no comment prose in any vehicle, a `.cypher` header included. What survives of the row is the static-check assertion, which is the half a gate runs. The header form a rule now carries is one line naming the file and the clauses it encodes, and the invariant and its inversion are read where they are ratified rather than in a copy beside the query. The retirement is not a judgement that the documentation was wrong: it was measurably unkept, corrected three times in cfdb PRs #709, #710 and #711 as the resolver vocabulary widened, each time by a reader who had to notice the drift unaided, because `scripts/comment_fence.py` walks only `.rs` files, `cfdb_query::parse` discards comments, and the CI smoke loop reads a `.cypher` header only for a `// smoke-skip:` marker. No mechanism ever read the prose this row ordered, so nothing could hold it true.
...
Tests:
- Unit: For each new `.cfdb/queries/arch-ban-rfc-<n>-<topic>.cypher`, a static-check assertion in `cfdb-query/tests/` (reusing the existing `predicate_schema_refs` test surface) confirms the file is a parseable single Cypher statement using only schema-vocabulary `:Labels` and `[:EdgeLabels]` from `cfdb_core`. Each Cypher file's header carries one line naming the file and the clauses it encodes, and no prose beside them (amended 2026-09-07, see header).
