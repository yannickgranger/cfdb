...
The cfdb / graph-specs duo (cfdb-033-cross-dogfood) makes this gap costlier than usual: graph-specs vendors cfdb as a pinned git dep and consumes its enrichment output. A silent enrichment regression becomes a silent graph-specs verdict regression on every downstream PR.
...
The Phase A query smoke test (issue #339) asserts query **liveness** — every `.cypher` file parses + executes against cfdb-self regardless of row count. cfdb-039-dogfood-enrichment-passes asserts **invariants** — specific row-count bounds.
...
**I6 — Smoke independence.** Failure in the Phase A smoke step does not skip the cfdb-039-dogfood-enrichment-passes invariant step; CI runs both unconditionally and surfaces both failures.
...
Each pass = 1 vertical-slice issue. Each issue carries the four-row `Tests:` template per project §2.5 (cfdb-033-cross-dogfood#3.5). All 7 link back to this RFC via `Refs: docs/cfdb-039-dogfood-enrichment-passes-dogfood-enrichment-passes.md` and to EPIC #338.
...
**Title:** `ci: dogfood-enrich harness — tools/dogfood-enrich binary, thresholds module, dogfood-determinism.sh (cfdb-039-dogfood-enrichment-passes#3.5)`
...
**Title:** `ci: self-enrich-deprecation — :Item.is_deprecated count vs source ground truth (cfdb-039-dogfood-enrichment-passes#3.1)`
...
**Title:** `ci: self-enrich-rfc-docs — :RfcDoc count >= docs/RFC-*.md count, REFERENCED_BY edges > 0 (cfdb-039-dogfood-enrichment-passes#3.1)`
...
**Title:** `ci: self-enrich-bounded-context — combined-pipeline coverage ≥MIN_BC_COVERAGE_PCT (cfdb-039-dogfood-enrichment-passes#3.1.1)`
...
- AC-4: CI step + README row.
- AC-5: Permanent-null crates (per I3.1) tracked in a filed issue with `Refs: cfdb-039-dogfood-enrichment-passes §I3.1`; the issue number is referenced in this PR's body.
...
**Title:** `ci: self-enrich-concepts — :Concept count == TOML distinct names, LABELED_AS > 0, conditional CANONICAL_FOR (cfdb-039-dogfood-enrichment-passes#3.1.2)`
...
**Title:** `ci: self-enrich-reachability nightly — ≥MIN_REACHABILITY_PCT reachable from :EntryPoint (cfdb-039-dogfood-enrichment-passes#3.1)`
...
**Title:** `ci: self-enrich-metrics nightly — ≥MIN_METRICS_COVERAGE_PCT non-null cyclomatic + unwrap_count (cfdb-039-dogfood-enrichment-passes#3.1)`
...
**Title:** `ci: self-enrich-git-history nightly — ≥MIN_GIT_COVERAGE_PCT non-null git_last_commit_unix_ts (cfdb-039-dogfood-enrichment-passes#3.1)`
...
- cfdb-029-code-facts-database addendum §A2.2 (the seven enrichment-pass row table)
- cfdb-036-cfdb-v2#3.3 (`enrich-metrics` producer / issue #203)
- cfdb-033-cross-dogfood#3.5 (four-row Tests template)
- cfdb-030-anti-drift-gate#3.2 (X-ray gate / self-audit step pattern)
