//! Query AST — the interchange format between parser and evaluator.
//!
//! Both the chumsky Cypher-subset parser (cfdb-query::parser) and the Rust
//! builder API (cfdb-query::builder) produce `Query` values. The store
//! evaluator (`StoreBackend::execute`) consumes them. This decoupling is the
//! architectural invariant: backend-swappable by construction.
//!
//! The AST intentionally covers only the v0.1 Cypher subset. Extensions
//! belong in v0.2 as additive enum variants — no struct field reshuffles,
//! no breaking changes to existing callers.
//!
//! Submodules:
//! - [`ast`] — core AST node types (`Query`, `Pattern`, `Predicate`, `Expr`,
//!   projections, aggregations, ordering).
//! - [`item_kind`] — `ItemKind` vocabulary.
//!
//! Moved to `cfdb-query` (verb-level composers and debt taxonomy do not
//! belong in the most-stable crate):
//! - `inventory` (`DebtClass`, `ScopeInventory`, `Finding`, `CanonicalCandidate`,
//!   `ReachabilityEntry`, `UnknownDebtClass`) — now `cfdb_query::inventory::*`.
//! - `list_items` (`list_items_matching` composer) — now `cfdb_query::list_items`.
//!
//! `ItemKind` is deliberately retained here — the schema-vs-verb question
//! is deferred to v0.2 schema design.

pub mod ast;
pub mod item_kind;

pub use ast::{
    Aggregation, CompareOp, Direction, EdgePattern, Expr, NodePattern, OrderBy, ParamBinding,
    PathPattern, Pattern, Predicate, Projection, ProjectionValue, Query, ReturnClause, WithClause,
};
pub use item_kind::{ItemKind, UnknownItemKind};
