//! CLEAN-3 architecture test for cfdb-enrich (RFC-056).
//!
//! cfdb-enrich hosts the strangler-fig destination for cfdb-petgraph's 7
//! enrichment passes, coded against the `GraphBackend`/`GraphView` port.
//! RFC-056 §2/§3: it depends ONLY on cfdb-core — never on cfdb-petgraph or
//! any other concrete storage engine, which would defeat the point of the
//! port (an enrichment pass swappable across backends).
...
const CRATE_SECTION: &str = "cfdb-enrich";
...
#[test]
fn cfdb_enrich_has_no_forbidden_dependencies() {
    ...
    assert!(
        forbidden.is_empty(),
        "cfdb-enrich/Cargo.toml [dependencies] contains forbidden crates: {forbidden:?}\n\
         cfdb-enrich must depend only on cfdb-core — the GraphBackend port \
...
#[test]
fn cfdb_enrich_dependencies_are_all_whitelisted() {
    let deps = parse_dependency_names();
    ...
    assert!(
        unknown.is_empty(),
        "cfdb-enrich/Cargo.toml [dependencies] contains crates not in the CLEAN-3 whitelist: {unknown:?}\n\
         Allowed: {allowed:?}\n\
         Update the [cfdb-enrich] section of .cfdb/workspace-dep-rules.toml AND \
         justify why the crate is enrichment-layer in a comment there."
    );
}

#[test]
fn cfdb_enrich_depends_on_cfdb_core() {
    let deps = parse_dependency_names();
    assert!(
        deps.contains("cfdb-core"),
        "cfdb-enrich must depend on cfdb-core (GraphBackend/EnrichBackend traits live there)"
    );
}
